...
#define SCE_IME_ENTER_LABEL_SEND INT32_C(1)
#define IME_TEXT_CHARACTERS 255U
...
static gpt_ime_result_fn result_callback;
...
static void start_requested(void)
{
    ...
    utf8_to_utf16("Ask anything...", placeholder, sizeof(placeholder) / sizeof(placeholder[0]));
    utf8_to_utf16("Message ProsperoAI", title, sizeof(title) / sizeof(title[0]));
    sce_ime_dialog_param_t param{};
    param.user_id = user_id;
    param.type = 0;
    param.enter_label = SCE_IME_ENTER_LABEL_SEND;
...
{
    if (requested && !gpt_input_pressed(GPT_INPUT_CROSS))
        start_requested();
    if (!active)
        return;
